...
import { useMyDeliveriesQuery } from '../api/marketplaceDeliveriesApi';

export const MyDeliveriesPage: React.FC = () => {
  const [statusFilter, setStatusFilter] = useState<string>('ALL');
  const [page, setPage] = useState<number>(0);

  const { data, isLoading, isError, refetch } = useMyDeliveriesQuery(statusFilter, page, 20);

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'CREATED':
        return <span className="px-2.5 py-0.5 bg-amber-950/80 border border-amber-700/60 text-amber-300 text-[11px] font-semibold rounded-full">REQUEST CREATED</span>;
      case 'ASSIGNED':
      ...
      case 'DELIVERED':
        return <span className="px-2.5 py-0.5 bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-[11px] font-semibold rounded-full font-bold">DELIVERED</span>;
      case 'CANCELLED':
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
              <span>🚚 My Deliveries & Fulfillment</span>
            </h1>
            <p className="text-xs text-slate-400 mt-1">Track outgoing shipments for your placed orders.</p>
          </div>
        ...
        {/* Loading State */}
        {isLoading && (
          <div className="text-center py-16 text-xs text-emerald-400">Loading deliveries...</div>
        )}

        {/* Error State */}
        {isError && (
          <div className="bg-rose-950/60 border border-rose-800 p-6 rounded-2xl text-center space-y-3">
            <p className="text-xs text-rose-300 font-medium">Failed to load delivery records.</p>
        ...
        {!isLoading && !isError && data?.items.length === 0 && (
          <div className="bg-slate-900/60 border border-slate-800 p-12 rounded-2xl text-center space-y-3">
            ...
            <h3 className="text-sm font-bold text-slate-300">No Deliveries Found</h3>
            <p className="text-xs text-slate-400">You have no active or historical deliveries under this status.</p>
          </div>
        )}
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <div className="text-xs text-slate-300">
                    ...
                    {delivery.trackingReference && (
                      <span className="ml-3 text-slate-400">
                        Tracking Ref: <strong className="font-mono text-slate-200">{delivery.trackingReference}</strong>
                      </span>
                    )}
                  </div>
                  ...
                  <div className="text-[11px] text-slate-400">
                    ...
                    {delivery.deliveryAddress
                      ? `${delivery.deliveryAddress.village || ''}, ${delivery.deliveryAddress.mandal || ''}, ${delivery.deliveryAddress.district || ''}`
                      : 'Address on record'}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <Link
                    ...
                    className="px-4 py-2 bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 hover:bg-emerald-900 text-xs font-medium rounded-xl transition-colors"
                  >
                    View Details & Track →
                  </Link>
                </div>
