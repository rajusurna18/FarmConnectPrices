...
export const MyOffersPage: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch } = useMyOffersQuery(statusFilter, page, 20);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PENDING':
        return <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-amber-950/80 text-amber-300 border border-amber-700/60">Pending Review</span>;
      case 'COUNTERED':
        return <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-cyan-950/80 text-cyan-300 border border-cyan-700/60">Counter Received</span>;
      case 'ACCEPTED':
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-emerald-900/40 pb-5">
          <div>
            <h1 className="text-2xl font-bold text-emerald-300 flex items-center gap-2">
              <span>🏷️ My Submitted Offers</span>
            </h1>
            <p className="text-xs text-emerald-400/80 mt-1">
              Track and negotiate marketplace offers submitted against farmer listings.
            </p>
          </div>
          <Link
            to="/marketplace"
        ...
        {/* Content */}
        {isLoading ? (
          <div className="text-center py-12 text-emerald-400 text-sm">Loading offers...</div>
        ) : isError ? (
          ...
          <div className="bg-emerald-950/40 border border-emerald-800/40 rounded-2xl p-12 text-center text-slate-400 text-sm">
            No submitted offers found for the selected filter.
          </div>
        ) : (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-emerald-400/80 mt-0.5">
                      Farmer Listing Reference Asking Price: ₹{offer.askingPriceReference?.toLocaleString('en-IN')} / {offer.priceUnit}
                    </p>
                  </div>

                  <div className="text-right">
                    <span className="text-xs text-slate-400 block">Submitted / Current Proposal</span>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-emerald-900/40 text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      View History Timeline ({offer.rounds?.length || 1})
                    </Link>
                    ...
                    {offer.canAccept && (
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-xl transition-colors"
                      >
                        Accept Counter
                      </button>
                    )}
                    ...
                    {offer.canCounter && (
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-amber-600 hover:bg-amber-500 text-white font-medium rounded-xl transition-colors"
                      >
                        Counter Proposal
                      </button>
                    )}
                    ...
                    {offer.canCancel && (
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium rounded-xl transition-colors"
                      >
                        Cancel Offer
                      </button>
                    )}
